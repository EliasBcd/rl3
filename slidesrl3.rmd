...
BibOptions(check.entries = FALSE,
           ...
           hyperlink = TRUE,
           dashed = FALSE)
myBib <- ReadBib("./Bibliographie.bib", check = FALSE)
```
...
- [Installer R sous Mac](https://cloud.r-project.org/bin/macosx/) (Pour les possesseurs de MAC récents, à processeurs M1 ou M2, il faut utiliser la version *arm64*, les autres doivent prendre la version classique)
...
Pour installer RStudio, rendez-vous sur [la page de téléchargement du logiciel](https://www.rstudio.com/products/rstudio/download/#download) et installez la version Desktop (gratuite) adaptée à votre système.
Notez qu'il faut installer R **avant** d'installer Rstudio, sinon Rstudio soulèvera une erreur.
...
- `r Citet(myBib, "Barnier")`
- `r Citet(myBib, "swirldev")`

Pour aller plus loin dans votre maîtrise de R : <https://swirlstats.com/swirlify/index.html> (en anglais)
...
Une fois installé, pour lancer Rstudio, vous pouvez cliquer sur l'icône correspondante ![Logo RStudio](RStudio-Logo-Flat.png){width=20%}
...
Nous allons maintenant indiquer à Swirl que nous sommes francophones

```{r french, eval=FALSE}
swirl::select_language("french", append_rprofile = T)
```

Et enfin nous allons télécharger la leçon 

```{r lecon, eval=F}
swirl::install_course_github("EliasBcd", "InitiationR")
```

---
## Lancement de Swirl

Pour lancer le cours interactif, il suffit taper 

```{r, eval=F}
swirl()
```

Swirl va nous proposer de choisir un pseudo.  
Utilisez votre nom et votre prénom (sans espace).

Il va maintenant nous proposer de choisir un cours, choisissez "InitiationR" (tapez le numéro correspondant dans la console)

Il nous demande maintenant de choisir une leçon.

Choisissez la première.  
Le reste se déroule maintenant dans votre console ! Laissez Swirl vous parler.
...
Pour conserver le résultat d'une opération, on peut le stocker dans un objet à l'aide de l'opérateur d'assignation `<-`.
