...
::: {.callout-tip}
`count()` affiche les valeurs manquantes comme une modalité à part entière. C'est un avantage sur la fonction `table()` de base, qui les masque par défaut — et qui vous ferait donc calculer des pourcentages sur un total incomplet.
:::
...
::: {.callout-note}
`.groups = "drop"` supprime le regroupement une fois le calcul fait. Sans cet argument, R affiche un avertissement et le tableau reste groupé, ce qui fausse les opérations suivantes — une erreur silencieuse de plus.
:::
